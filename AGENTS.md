...
**RAW decode concurrency is capped:** `get_photo_file`'s RAW arm holds a 4-permit `RAW_DECODE_LIMIT` semaphore across decode+encode (each decode transiently holds several full-resolution buffers). Non-RAW photo files and collage images stream via `tokio_util::io::ReaderStream` + `warp::reply::stream` with a re-statted content-length — never buffer whole files.
