...
class PhotoViewer {
    ...
    updateNavigation() {
        ...
        if (this.elements.next) {
            this.elements.next.style.display = this.currentIndex < this.photos.length - 1 ? 'block' : 'none';
        }
    }
    ...
    updatePhotoInfo() {
        ...
        if (this.elements.date) {
            this.elements.date.textContent = photo.date_taken ? 
                utils.formatDate(photo.date_taken) : 'Unknown';
        }
        ...
        if (this.elements.camera) {
            const camera = photo.camera_make && photo.camera_model ? 
                `${photo.camera_make} ${photo.camera_model}` : 'Unknown';
            this.elements.camera.textContent = camera;
        }

        // Location
        if (this.elements.location) {
            const location = photo.gps_latitude && photo.gps_longitude ? 
                `${photo.gps_latitude.toFixed(6)}, ${photo.gps_longitude.toFixed(6)}` : 'No location data';
