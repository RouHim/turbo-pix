...
test.describe('Mobile Search', () => {
  ...
  test('should toggle mobile search', async ({ page }) => {
    ...
    const exists = (await mobileSearchBtn.count()) > 0;
    ...
    // Mobile search container should be visible
    const mobileSearchContainer = page.locator('.mobile-search');
    await expect(mobileSearchContainer).toBeVisible();
  });
  ...
  test('should perform search on mobile', async ({ page }) => {
    ...
    if (!exists) {
      test.skip('Mobile search not available');
    }
    ...
    const mobileSearchInput = page.locator('.mobile-search input');
