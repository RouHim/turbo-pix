...
async fn main() -> Result<()> {
    ...
    for video_path in videos {
        println!("\nBenchmarking video: {:?}", video_path.file_name().unwrap());
        let video_path_str = video_path.to_string_lossy().to_string();

        let mut embeddings = Vec::new();

        println!("{:<10} | {:<15} | {:<15}", "Frames", "Time (ms)", "FPS equivalent");
        println!("{:-<10}-+-{:-<15}-+-{:-<15}", "", "", "");
        ...
        for &count in &frame_counts {
            // Clear cache for this video to force re-computation
             ...
             if let Some(id) = id {
                 ...
                 conn.execute("DELETE FROM semantic_vector_path_mapping WHERE id = ?", [id])?;
             }
             conn.execute("DELETE FROM video_semantic_metadata WHERE path = ?", [&video_path_str])?;
             drop(conn);

            let start = Instant::now();
            engine.compute_video_semantic_vector(&video_path_str, Some(count))
                .await
